...
def test_basic_keys():
    ...
    # Elements with keys
    keyed_elements = [h.span(key="1")["1"], h.span(key="2")["2"], h.span(key="3")["3"]]
...
def test_simple_dom_identity():
    ...
    assert first_span.isSameNode(second_span), (
        "Single element should preserve DOM node identity on re-render"
    )
...
def test_key_reordering():
    ...
    def KeyedList(ctx, props):
        for props in ctx:
            ...
            yield h.div[[h.span(key=key)[f"Item {key}"] for key in order]]
    ...
    assert reordered_spans[1].isSameNode(item1_node), (
        "Item 1 DOM node should maintain identity"
    )
    assert reordered_spans[2].isSameNode(item2_node), (
        "Item 2 DOM node should maintain identity"
    )
    assert reordered_spans[0].isSameNode(item3_node), (
        "Item 3 DOM node should maintain identity"
    )
...
def test_keyed_arrays():
    ...
    assert modified_lis[0].isSameNode(apple_node), (
        "Apple DOM node should maintain identity"
    )
    assert modified_lis[1].isSameNode(cherry_node), (
        "Cherry DOM node should maintain identity"
    )
...
def test_mixed_keyed_unkeyed():
    ...
    renderer.render(
        h.div[
            h.span["Unkeyed"], h.span(key="keyed")["Keyed"], h.span["Another unkeyed"]
        ],
        document.body,
    )
...
def test_selective_copy_props():
    ...
    selective = h.div(
        copy="style class", style="color: red;", className="greeting", id="test"
    )["Content"]
...
def test_copy_prop_exclusion():
    ...
    excluded = h.div(
        copy="!style !class", style="color: blue;", className="excluded", id="included"
    )["Content"]
...
def test_complex_key_scenarios():
    ...
    scenario1 = h.div[h.span(key="a")["A"], h.span(key="b")["B"], h.span(key="c")["C"]]

    # Reverse order
    scenario2 = h.div[h.span(key="c")["C"], h.span(key="b")["B"], h.span(key="a")["A"]]
...
def test_ref_with_special_elements():
    ...
    from crank import Portal, Raw, h
    ...
    # Ref with Portal
    portal_with_ref = h(Portal, ref=ref_callback, root="body")[h.div["Portal content"]]
