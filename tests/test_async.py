...
def test_async_imports():
    ...
    from crank.async_ import Suspense, SuspenseList, lazy
...
async def test_async_generator_complex():
    ...
    result1 = await renderer.render(
        h(ComplexAsyncGenerator, mode="normal"), document.body
    )
    assert result1 is not None

    # Test loading mode
    result2 = await renderer.render(
        h(ComplexAsyncGenerator, mode="loading"), document.body
    )
    assert result2 is not None

    # Test multiple renders to verify state progression
    result3 = await renderer.render(
        h(ComplexAsyncGenerator, mode="normal"), document.body
    )
...
async def test_for_await_of_loading_sequence():
    ...
    try:
        ...
        result = await renderer.render(
            h(LoadingSequence, message="async-data"), document.body
        )
        assert result is not None
    except NameError:
        ...
        try:
            ...
            render_root = None
        result = await renderer.render(
            h(LoadingSequence, message="async-data"), render_root
        )
        assert result is not None
