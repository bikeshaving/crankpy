...
def test_nested_components():
    ...
    def OuterComponent(ctx, props):
        ...
        for _ in ctx:
            yield h.div[h.h1[title], h(InnerComponent, text="nested")]
...
def test_component_with_children():
    ...
    def WrapperComponent(ctx, props):
        for _ in ctx:
            yield h.div(className="wrapper")[h.h2["Wrapper"], props.get("children", [])]

    # Clear DOM and render component with children
    document.body.innerHTML = ""
    renderer.render(
        h(WrapperComponent, children=[h.p["Child 1"], h.p["Child 2"]]), document.body
    )
