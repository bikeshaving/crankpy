...
def test_generator_with_props_changes():
    ...
    def PropsChangeGenerator(ctx, props):
        ...
        for props in ctx:
            ...
            renders.append(name)
            yield h.div[
                h.h3[f"Current: {name}"], h.ul[[h.li[render] for render in renders]]
            ]
...
def test_nested_generators():
    ...
    def OuterGenerator(ctx, props):
        for props in ctx:
            ...
            yield h.div[
                h.h2["Outer Generator"], h(InnerGenerator, value=multiplier * 2)
            ]
    ...
    def ErrorHandlingGenerator(ctx, props):
        ...
        for props in ctx:
            try:
                ...
                errors.append(str(e))
                yield h.div[
                    h.p["Error occurred"], h.ul[[h.li[error] for error in errors]]
                ]
